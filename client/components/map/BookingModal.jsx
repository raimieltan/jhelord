...
const filterCarsWithinRadius = (carOptions, currentLocation, radius) => {
  return carOptions.filter((car) => {
    const distance = calculateDistance(
      currentLocation.lat,
      currentLocation.lng,
      car.unit[0]?.location?.latitude,
      car.unit[0]?.location?.longitude
    );
...
const BookingModal = ({
    ...
    try {
      if (user) {
        const response = await fetch(
          `https://jhelord-backend.onrender.com/api/bookings/user/${user}`,
          {
            method: "GET",
          }
        );
    ...
    const fetchBookingStatus = async () => {
      try {
        if (bookingId) {
          ...
          const response = await fetch(
            `https://jhelord-backend.onrender.com/api/bookings/${bookingId}`,
            {
              method: "GET",
              headers: {
  ...
  const fetchDrivers = async () => {
    ...
    try {
        ...
        setFilteredDrivers(filterCarsWithinRadius(onlineDrivers, pickupLocation, radius));
    } catch (err) {
  ...
  const fetchOnlineUserIds = async () => {
    try {
      const response = await fetch(
        "https://jhelord-backend.onrender.com/api/onlineUsers"
      );
  ...
  useEffect(() => {
    const fetchDrivers = async () => {
      ...
      try {
        const response = await fetch(
          "https://jhelord-backend.onrender.com/api/drivers/"
        );
        const data = await response.json();

        // Filter drivers based on online user IDs
        const onlineUserIds = await fetchOnlineUserIds();
        const onlineDrivers = data.filter((driver) =>
          onlineUserIds.includes(driver.User.id.toString())
        );
  ...
  useEffect(() => {
    if (pickupLocation && driversData.length > 0) {
      const filtered = filterCarsWithinRadius(
        driversData,
        pickupLocation,
        radius
      );
  ...
  const handleBooking = async (driverId) => {
    try {
      ...
      setUser(userId);
      const response = await fetch(
        "https://jhelord-backend.onrender.com/api/bookings",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const changeBookingStatus = async (bookingId) => {
    try {
      ...
      const response = await fetch(
        `https://jhelord-backend.onrender.com/api/bookings/${bookingId}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const CarItem = ({ unit, driverName, driver }) => (
    <View style={styles.itemContainer}>
      <Image
        source={{
          uri: `https://jhelord-backend.onrender.com/uploads/${
            driver.User.profileImage.split("/")[2]
          }`,
        }}
      ...
      <View style={styles.carDetails}>
        ...
        <Text style={styles.driverName}>
          {calculateDistance(
            pickupLocation.lat,
            pickupLocation.lng,
            driver.unit[0]?.location?.latitude,
            driver.unit[0]?.location?.longitude
          ).toFixed(2) + " km away"}
        </Text>
        <StarRatingDisplay
          starSize={16}
